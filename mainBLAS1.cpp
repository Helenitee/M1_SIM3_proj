      ...
      for (int k = 0; k < K; ++k) {
        ...
        for (int n = 0; n < Np; ++n) {
          ...
          for (int f = 0; f < NFacesTet; ++f) {
            ...
            int offset = n * NFacesTet * Nfp + f * Nfp;

            _rhsQ[id + 0] -= Fscale * cblas_ddot(Nfp, &_LIFT[offset], 1, &s_p_flux[f * Nfp], 1);
            _rhsQ[id + 1] -= Fscale * cblas_ddot(Nfp, &_LIFT[offset], 1, &s_u_flux[f * Nfp], 1);
            _rhsQ[id + 2] -= Fscale * cblas_ddot(Nfp, &_LIFT[offset], 1, &s_v_flux[f * Nfp], 1);
            _rhsQ[id + 3] -= Fscale * cblas_ddot(Nfp, &_LIFT[offset], 1, &s_w_flux[f * Nfp], 1);
          }
        }
      }
